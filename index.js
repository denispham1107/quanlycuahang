...
const MAX_CLIENT_STATE_CHARS = Number(process.env.MAX_AI_CLIENT_STATE_CHARS || 180000);
...
function normalizeClientStateSnapshot(value) {
  ...
  try {
    ...
    return {
      ...
      error: "Client state snapshot is not serializable."
    };
  }

  if (serialized.length <= MAX_CLIENT_STATE_CHARS) {
    return value;
  }
  ...
  return {
    source: "frontend_json_export_for_ai",
    truncated: true,
    maxChars: MAX_CLIENT_STATE_CHARS,
    note: "Client JSON export was larger than MAX_AI_CLIENT_STATE_CHARS, so only a preview is included.",
    dataPreview: serialized.slice(0, MAX_CLIENT_STATE_CHARS)
  };
}
...
function normalizeAIFileAttachments(value) {
  ...
  for (const file of value.slice(0, MAX_ATTACHMENT_COUNT)) {
    ...
    const type = String(file.type || "").slice(0, 120);
    const kind = String(file.kind || "").slice(0, 40);
    const size = Number.isFinite(Number(file.size)) ? Number(file.size) : 0;
    const extractionNote = String(file.extractionNote || "").slice(0, 120);
    let text = typeof file.text === "string" ? file.text : "";
    if (!text.trim()) {
      text = "Khong co noi dung van ban duoc trich xuat tu file nay.";
    }

    const limit = Math.max(0, remainingChars);
    const clippedText = text.slice(0, limit);
    ...
    files.push({
      ...
      size,
      text: clippedText,
      truncated: Boolean(file.truncated) || clippedText.length < text.length,
      extractionNote
    });
...
exports.chatWithAI = onRequest(
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (clientState) {
        context.clientExportJson = clientState;
      }
      const attachments = normalizeAIFileAttachments(req.body?.attachments);
      if (attachments.length) {
        context.uploadedFiles = attachments;
      }
      const systemPrompt = [
        ...
        "Náº¿u context cÃ³ clientExportJson, hÃ£y Æ°u tiÃªn Ä‘á»c nÃ³ vÃ¬ Ä‘Ã³ lÃ  báº£n JSON frontend vá»«a xuáº¥t ra tá»« toÃ n bá»™ dá»¯ liá»‡u hiá»‡n táº¡i cá»§a website.",
        "Náº¿u context cÃ³ uploadedFiles, hÃ£y Ä‘á»c ná»™i dung file Ä‘Ã­nh kÃ¨m Ä‘Ã³ Ä‘á»ƒ tráº£ lá»i cÃ¢u há»i. Náº¿u file khÃ´ng trá»‹ch xuáº¥t Ä‘Æ°á»£c ná»™i dung, hÃ£y nÃ³i rÃµ vÃ  khÃ´ng bá»‹a dá»¯ liá»‡u trong file.",
